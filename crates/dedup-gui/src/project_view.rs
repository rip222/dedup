...
impl ProjectView {
    ...
    pub fn copy_group_as_llm_prompt(
        ...
        let prompt = crate::llm_prompt::llm_prompt(&group, &occurrences);
...
fn render_group_toolbar(state: &AppState, group_id: i64) -> gpui::Div {
    ...
    // Issue #57 — "Copy as LLM prompt". The clipboard payload is
    // path-only (file:start-end refs), so file readability no longer
    // affects the button — the receiving agent reads the files itself.
    let gid_llm = group_id;
    let llm_btn = with_tooltip(
        div()
        ...
        "Copy a markdown prompt with every occurrence's path:line range to the clipboard",
    );
